...
class Random_walk_edit_Calculator(GEDLIB_Calculator):
    def __init__(self, decay_lambda=0.1,max_walk_path_length=5, **kwargs):
        super().__init__(**kwargs)
        self.decay_lambda = decay_lambda
        self.max_walk_path_length = max_walk_path_length
        self.inf_walk_matrix = None
        self.approx_walk_matrix = None
    ...
    def run_method(self, graph1_index, graph2_index):
        ...
        start_time = time()
        self.inf_walk_matrix[graph1_index][graph2_index] = self.infinite_length_random_walk_similarity(product_graph, llamda=self.decay_lambda)
        self.inf_walk_matrix[graph2_index][graph1_index] = self.inf_walk_matrix[graph1_index][graph2_index]
        self.approx_walk_matrix[graph1_index][graph2_index] = self.limited_length_approx_random_walk_similarity(product_graph, llamda=self.decay_lambda, max_length=self.max_walk_path_length,kernel_array=self.approx_walk_matrix[graph1_index][graph2_index]) 
        self.approx_walk_matrix[graph2_index][graph1_index] = self.approx_walk_matrix[graph1_index][graph2_index]
        random_walk_time = time() - start_time
        self.sum_random_walk_time += random_walk_time
    def activate(self):
        self.inf_walk_matrix = np.zeros((self.size, self.size))
        self.approx_walk_matrix = np.zeros((self.size, self.size, self.max_walk_path_length+1))        
        super().activate()

        if DEBUG:
            print(f"Activated Random_walk_edit_Calculator with decay_lambda={self.decay_lambda} and max_walk_path_length={self.max_walk_path_length}")
    ...
    def limited_length_approx_random_walk_similarity(self, product_graph: nx.Graph, llamda=0.1, max_length=10,kernel_array=None):
        if kernel_array is None:
            raise ValueError("kernel_array must be provided for limited length random walk similarity.")
        if kernel_array.shape[0] != max_length+1:
            raise ValueError("kernel_array shape does not match max_length.")
        ...
        for step in range(max_length):
            ...
            # kernel_array[step] = total_similarity
            walk_distribution = np.dot(walk_distribution, transition_matrix)
        return kernel_array

    def infinite_length_random_walk_similarity(self,product_graph: nx.Graph, llamda=0.1):
