...
import random
# Class to test the protype selector.
class Base_Calculator:
    def __init__(self, size=100):
        self.size = size
        self.dataset = []
        for i in range(size):
            # random coordinates (floats in the range of 0 to 100)
        ...
        self.dataset_indexes = list(range(size))
        # split into 80/20 train/test
        # randomly shuffled
        self.train_set = random.sample(self.dataset_indexes, int(size * 0.8))
        self.test_set = [item for item in self.dataset_indexes if item not in self.train_set]

    def get_distance(self, index1, index2):
        p1 = self.dataset[index1]
        p2 = self.dataset[index2]
        # return the Eucledian distance between the two points
        return ((p1[0] - p2[0]) ** 2 + (p1[1] - p2[1]) ** 2) ** 0.5


    def get_complete_matrix(self, method="Mean-Distance", x_graphindexes=None):
        """
        This method should be implemented in the subclass to return a complete distance matrix
        for the graphs in the dataset.
        """
        distance_matrix = np.zeros((len(x_graphindexes), len(x_graphindexes)))
        for i, idx1 in enumerate(x_graphindexes):
            for j, idx2 in enumerate(x_graphindexes):
                if i == j:
                    if idx1 != idx2:
                        raise ValueError(f"Index {idx1} and {idx2} should be equal, but are not.")
                    distance_matrix[i][j] = 0
                else:
                    # calculate the distance between the two graphs
                    distance_matrix[i][j] = self.get_distance(idx1, idx2)
...
def select_k_Centers(G, ged_calculator:Base_Calculator, ged_distance="Mean-Distance", size=3,initial_prototype_selector=select_RPS):
    ...
    # get the indexes of T in whicht the prototypes P are located
    Pi = []
    for p in P:
        for index, g in enumerate(T):
            if p == g:
                Pi.append(index)
                break
            # 2.
    ...
    while another_iteration:
        S_list = [[p_id] for p_id in Pi]
        for t_id,g in enumerate(T):
            if g not in P:
                # find the nearest prototype
                distances_to_prototypes = [distance_matrix[t_id, p_id] for p_id in Pi]
                nearest_prototype_index = np.argmin(distances_to_prototypes)
                S_list[nearest_prototype_index].append(t_id)
