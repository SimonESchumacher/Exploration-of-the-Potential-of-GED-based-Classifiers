...
from graph_mixup.mixup_generation.if_mixup.hyperparams import (
    IfMixupMethodHPProvider,
)


class IfMixupDataModuleProvider(AbstractDataModuleProvider):
    @override
    def _get_method_config(self) -> BaseConfig:
        method_hp_provider = IfMixupMethodHPProvider(self.trial)
    ...
    def get_data_module(
        ...
        return RDBDataModule(
            ...
            method_name=PreBatchMixupName.IF_MIXUP,
            inner_fold=inner_fold,
        )


class IfMixupSEDataModuleProvider(IfMixupDataModuleProvider):
    @override
    def get_data_module(
        self, inner_fold: int | None
    ) -> RDBDataModule[
        ...
        return RDBDataModule(
            config=self._get_data_module_config(),
            method_name=PreBatchMixupName.IF_MIXUP_SE,
            inner_fold=inner_fold,
        )
