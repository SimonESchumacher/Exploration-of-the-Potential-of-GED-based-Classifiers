...
select e.id                                              as experiment_id,
       ...
       s.best_avg_val_acc                                AS best_avg_val_acc_fold,
       avg(r.test_acc) - s.best_avg_val_acc              AS avg_acc_diff_fold,
       s.best_avg_val_acc_epoch                          AS best_avg_val_acc_epoch_fold,
       s.best_avg_train_loss                             AS best_avg_train_loss_fold,
       s.best_avg_train_loss_epoch                       AS best_avg_train_loss_epoch_fold,
       s.fold                                            AS fold,
...
from experiments e
         ...
         join `iota_opt`.completed_trials_per_study ctps
...
select results_per_fold.experiment_id,
       ...
       avg(results_per_fold.best_avg_val_acc_fold)         AS avg_best_avg_val_acc,
       stddev_samp(results_per_fold.best_avg_val_acc_fold) AS std_best_avg_val_acc,
       stddev_samp(results_per_fold.best_avg_val_acc_fold) /
       sqrt(count(results_per_fold.fold))                  AS std_err_best_avg_val_acc,
       avg(results_per_fold.avg_test_acc_fold) -
       avg(results_per_fold.best_avg_val_acc_fold)         as avg_acc_diff,
       avg(results_per_fold.best_avg_val_acc_epoch_fold)   AS avg_best_avg_val_acc_epoch,
...
create or replace view hyperparams_per_fold as
select e.id        as experiment_id,
       s.optuna_study_id,
       e.dataset_name,
       e.model_name,
...
create or replace view hyperparams_overall as
select e.id                 as experiment_id,
       e.dataset_name,
       e.model_name,
       e.method_name,
       ...
       required_folds,
       count(`value`)       as cnt
from hyperparams_per_fold mhpf
