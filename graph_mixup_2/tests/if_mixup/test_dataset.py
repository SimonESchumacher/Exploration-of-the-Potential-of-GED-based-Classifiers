...
from graph_mixup.mixup_methods.if_mixup.dataset import IfMixupDataset
from tests.dataset_base import MixupDatasetBaseTest


class TestIfMixupDataset(MixupDatasetBaseTest):
    @pytest.fixture
    def mixup_dataset(self, vanilla_dataset: Dataset) -> Dataset:
        return IfMixupDataset(vanilla_dataset)

    def test_no_one_hot_encoding_raises_error(self):
        with pytest.raises(AssertionError):
            IfMixupDataset(TUDataset("data", "PROTEINS"))

    def test_len(self, vanilla_dataset):
        mixup_dataset = IfMixupDataset(vanilla_dataset, augmented_ratio=0.0)
        assert len(mixup_dataset) == len(vanilla_dataset)

        mixup_dataset = IfMixupDataset(vanilla_dataset, augmented_ratio=0.5)
        assert len(mixup_dataset) == int(1.5 * len(vanilla_dataset))

        mixup_dataset = IfMixupDataset(
