...
class Trivial_GED_SVC(Base_GED_SVC):
    ...
    def compare(self, g1, g2):
        if self.similarity_function == 'k1':
            return -(self.ged_calculator.compare(g1, g2, method=self.ged_bound) ** 2)
        elif self.similarity_function == 'k2':
            return -self.ged_calculator.compare(g1, g2, method=self.ged_bound)
        elif self.similarity_function == 'k3':
            return np.tanh(-self.ged_calculator.compare(g1, g2, method=self.ged_bound))
        elif self.similarity_function == 'k4':
            return np.exp(-self.ged_calculator.compare(g1, g2, method=self.ged_bound))
